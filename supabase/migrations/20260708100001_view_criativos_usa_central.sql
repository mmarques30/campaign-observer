-- mads_v_criativos_metricas passa a obter o CRM da função central mads_f_crm_por_ad
-- (janela fixa 30d, idêntica à anterior). Vídeo/semáforos inalterados.
CREATE OR REPLACE VIEW mads_v_criativos_metricas AS
 WITH agg AS (
         ...
         SELECT agg.ad_uuid, agg.meta_ad_id, agg.ad_nome, agg.ad_status, agg.adset_nome, agg.campanha_nome,
           ...
           FROM agg JOIN mads_f_crm_por_ad((CURRENT_DATE - 30), CURRENT_DATE) crm ON crm.ad_uuid = agg.ad_uuid
        )
