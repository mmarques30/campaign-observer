-- Corrige a atribuição CRM->ad na view mads_v_criativos_metricas usando DUPLA-CHAVE (ad + adset):
...
-- Isso resolve o descasamento anterior (contacts.utm_term não batia com mads_ads.meta_ad_id):
-- a chave confiável do ad está no form_submissions.raw_data->utm_content.
--
-- Ajustes em relação ao SQL enviado:
--   * deals.is_lost NÃO existe -> pipe_ativo usa (is_won IS NOT TRUE AND closed_at IS NULL).
--   * Sub-select de contatos DISTINTOS antes de agregar deals, pra o join de form_submissions
--     não multiplicar valor_ganho_brl (soma) — contagens usam COUNT(DISTINCT) e ficam corretas.
--   * Janela dinâmica CURRENT_DATE - 30 (casa com o gasto de 30d da view), no lugar da data fixa.
...
CREATE OR REPLACE VIEW mads_v_criativos_metricas AS
 WITH agg AS (
         ...
         SELECT agg.ad_uuid,
            ...
            agg.desde,
            agg.ate,
            COALESCE(crm.mql, 0::bigint) AS mql_no_crm,
            COALESCE(crm.sal, 0::bigint) AS sal_no_crm,
            COALESCE(crm.ganho, 0::bigint) AS ganhos_no_crm,
            ...
            COALESCE(crm.sql_qualif, 0::bigint) AS sql_no_crm,
                ...
                CASE
                    WHEN COALESCE(crm.mql, 0::bigint) > 0 THEN round(agg.gasto / crm.mql::numeric, 2)
                    ELSE NULL::numeric
                END AS cpmql_brl,
                CASE
                    WHEN COALESCE(crm.sal, 0::bigint) > 0 THEN round(agg.gasto / crm.sal::numeric, 2)
                    ELSE NULL::numeric
                END AS cpsal_brl
           FROM agg
             LEFT JOIN LATERAL (
                   ...
                   FROM (
                         SELECT DISTINCT ct.id
                          ...
                          WHERE ct.created_at >= (CURRENT_DATE - 30)
                            ...
                            AND (fs.raw_data ->> 'utm_content'::text = agg.meta_ad_id OR fs.raw_data ->> 'utm_content'::text = agg.utm_content_slug)
                        ) cts
