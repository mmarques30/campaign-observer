-- Exclui do alerta "Campanhas que precisam de atenção" campanhas pausadas há mais de 7 dias.
-- Regra: status = 'pausada' E pausada_em < now() - 7 dias => precisa_atencao = false.
-- Campanhas ativas (ou pausadas há <= 7 dias, ou sem pausada_em) seguem elegíveis.
-- Nenhum dado é apagado; a campanha continua na view, só deixa de ser sinalizada.

CREATE OR REPLACE VIEW mads_v_conversao_vs_crm AS
 WITH meta_perf AS (
         SELECT c.id AS campanha_uuid,
            ...
            c.utm_campaign,
            c.meta_campaign_id,
            c.orcamento_diario_centavos::numeric / 100.0 AS orcamento_diario_brl,
         ...
         SELECT mp.campanha_uuid,
            ...
            COALESCE(fd.form_submissions_30d, 0::bigint) AS form_submissions_30d,
                CASE
                    WHEN mp.lp_views_30d > 0::numeric THEN round(mp.leads_meta_30d::numeric / mp.lp_views_30d * 100::numeric, 2)
                    ELSE NULL::numeric
                END AS taxa_conversao_meta_pct,
                CASE
                    WHEN mp.lp_views_30d > 0::numeric THEN round(COALESCE(crm.contacts_crm_30d, 0::bigint)::numeric / mp.lp_views_30d * 100::numeric, 2)
                    ELSE NULL::numeric
                END AS taxa_conversao_crm_pct,
                CASE
                    WHEN mp.leads_meta_30d > 0 THEN round(mp.gasto_30d_brl / mp.leads_meta_30d::numeric, 2)
                    ELSE NULL::numeric
                END AS cpl_meta_brl,
                CASE
                    WHEN COALESCE(crm.contacts_crm_30d, 0::bigint) > 0 THEN round(mp.gasto_30d_brl / crm.contacts_crm_30d::numeric, 2)
                    ELSE NULL::numeric
                END AS cpl_crm_brl,
            ...
            abs(mp.leads_meta_30d - COALESCE(crm.contacts_crm_30d, 0::bigint)) AS divergencia_absoluta,
                CASE
                    ...
                    WHEN COALESCE(crm.contacts_crm_30d, 0::bigint) > 0 AND mp.leads_meta_30d = 0 THEN 'meta_nao_atribui'::text
 ...
 SELECT campanha_uuid,
    ...
    gasto_30d_brl,
    contacts_crm_30d,
    contacts_crm_meta_30d,
    form_submissions_30d,
    taxa_conversao_meta_pct,
    taxa_conversao_crm_pct,
    cpl_meta_brl,
    cpl_crm_brl,
    divergencia_meta_menos_crm,
    ...
    status_conexao,
        ...
        CASE
            -- Pausadas há mais de 7 dias saem do alerta (janela de reação já passou).
